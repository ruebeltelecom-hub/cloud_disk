...
class NCMainTabBarController: UITabBarController {
    ...
    private func configureMoreController() {
        ...
        // Убрана вкладка «События» (Activity): серверный модуль Activity не используется,
        // на Android такой вкладки тоже нет. В сториборде index 3 — это Activity.
        if controllers.indices.contains(3) {
